...
int portB_conf(int pin, enum port_mode mode)
{
	...
	if (mode == SIGOUT){
		...
		// dicho pin en el puerto B salga la se帽al correspondiente del
...
int portG_conf(int pin, enum port_mode mode)
{
	...
	switch (mode) {
		case INPUT:
			...
			// REVISAR: poner en rPCONG 00 a partir de la posici髇 pos para
			// configurar como pin de entrada el pin indicado por el par醡etro pin
			break;
		case OUTPUT:
			rPCONG = rPCONG & ~(0x2<<pos);
			rPCONG = rPCONG | (0x1<<pos);

			// REVISAR: poner en rPCONG 01 a partir de la posici髇 pos para
			// configurar como pin de salida el pin indicado por el par醡etro pin
			break;
		case SIGOUT:
			rPCONG = rPCONG & ~(0x3<<pos);
			rPCONG = rPCONG | (0x2<<pos);
			// REVISAR: poner en rPCONG 10 a partir de la posici髇 pos para
			// que salga la se脙卤al interna correspondiente por el pin indicado
			// por el par醡etro pin
			break;
		case EINT:
			rPCONG = rPCONG | (0x3<<pos);
			// REVISAR: poner en rPCONG 11 a partir de la posici髇 pos para
			// habilitar la generaci髇 de interrupciones externas por el pin
			// indicado por el par醡etro pin
			break;
		default:
			return -1;
	}
...
switch (trig) {
		case LLOW:
			...
			// REVISAR: poner en rEXTINT a partir de la posici贸n pos tres bits
...
int portG_read(int pin, enum digital* val)
{
	...
	/*REVISAR: true si esta a 1 en rPDATG el pin indicado por el par醡etro pin*/
	if (rPDATG & (0x1 << pin))
		*val = HIGH;
	else
		*val = LOW;
