...
int setup(void)
{
	...
	//REVISAR: utilizando el interfaz para el puerto G definido en gpio.h hay
...
int loop(void)
{
	...
	if (RL.moving) {
		...
		if (RL.iter == 0) {
			...
			if(RL.position == -1)
				RL.position = 5;
			// REVISAR: debemos hacer avanzar el led rotante una posición en
